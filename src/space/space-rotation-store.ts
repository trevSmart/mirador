/* Per-room (per-space) perspective rotation for the 3D space render, persisted
...
/** Load the whole `{ [spaceId]: {az, tilt} }` map, sanitized. */
...
/** Read a single space's rotation (falls back to the default isometric pose). */
export function loadRoomRotation(spaceId: string): RoomRotation {
  return loadRoomRotations()[spaceId] ?? defaultRotation()
}

/** Persist a single space's rotation without clobbering the other spaces'
    entries — each `SpaceView3D` instance owns only its own key. */
export function saveRoomRotation(spaceId: string, rotation: RoomRotation): void {
  const map = loadRoomRotations()
  map[spaceId] = sanitizeRotation(rotation)
